...
- チームメンバーとのディスカッション
    - 個人的な意見(現時点)
        ...
        ```
        とにかく、どのようなプロセスをえても、未知の薬剤に対応できるようなモデルが勝つと思います。
        ローカルで未知の薬剤に対応できているかを図るのであれば、赤い線から引き剥がさなくてはなりません。
        引き剥がされていても薬剤リークが原因であればそれはたまたまであり、良いモデルを作れたとは言えないと思います。
        LBで良いスコアが出るかは別として、ローカルでより良いモデル(未知の薬剤に対応できる)を評価するためには、薬剤リークは0であるべきだと思っています。
        薬剤リークが0である状態でもっとも良いCVを出すアーキテクチャ(特徴量やハイパラも含め)を見定め、
        サブするときには薬剤リーク有りの状態で再度モデルを作り直すのが最もLBが上がる手段かなと思っています。
        ```

### 20201110
- うーーーん。忙しくてできひんかった...。すみません。

### 20201112
- チョットダケディスカッションヨンダ

### 20201118
- 久々にKaggleやる！！。めちゃんこ忙しかった。  
...
- nb022
    ...
    - nb020のfoldを使う
    - top8を除く
    - smoothing の値をいろいろ変えてmodelを作成した。
        グラフ左: smothing = [10e-5, 10e-1.5] の範囲でモデルを作成したグラフ。縦軸はoof_scoreを表す。
        グラフ右: ミニマム付近の拡大図
        oof_scoreが最小となるsmoothingの値は、 `0.000853167852417281` だった。
        0.001をもともと使ってるので極端な性能向上にはならないとはおもいますが、試してみる価値はあり。。
